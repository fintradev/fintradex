...
### Prerequisites
- Rust 1.75 or higher
- Polkadot SDK Stable 2503
- Cargo and related tools
- Trading-specific development tools
...
cargo test --features trading-tests
...
### Chain Specification
Generate chain specifications using the `chain-spec-builder`:

```bash
# Development chain spec with trading features
...
# Production chain spec with trading optimization
chain-spec-builder create \
  --raw-storage \
  --relay-chain "rococo-local" \
  --para-id 1000 \
